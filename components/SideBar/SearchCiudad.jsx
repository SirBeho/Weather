...
export default function SearchCiudad({ pais, setCoord, setCiudad, setsearchPais, searchPais, setsearch }) {
  ...
  // Estado para almacenar la lista filtrada de ciudades
  const [filtrado, setFiltrado] = useState([]);
  // Estado para almacenar el valor del input de búsqueda
  const [localValue, setLocalValue] = useState("");
  // Estado para almacenar las coordenadas de la ciudad seleccionada
  ...
  // Estado para habilitar/deshabilitar el botón de búsqueda
  ...
  // Filtrar la lista de ciudades según el país y el nombre ingresado
  ...
  // Manejar el click en una ciudad de la lista filtrada
  ...
  // Icono de búsqueda
  const search = (
    <svg xmlns="http://www.w3.org/2000/svg" height="30" viewBox="0 -960 960 960" fill="#616475" width="30">
      <path d="M796-121 533-384q-30 26-69.959 40.5T378-329q-108.162 0-183.081-75Q120-479 120-585t75-181q75-75 181.5-75t181 75Q632-691 632-584.85 632-542 618-502q-14 40-42 75l264 262-44 44ZM377-389q81.25 0 138.125-57.5T572-585q0-81-56.875-138.5T377-781q-82.083 0-139.542 57.5Q180-666 180-585t57.458 138.5Q294.917-389 377-389Z" />
    </svg>
  );

  // Icono de país activo o Nombre de país
  const paisActive = (
    <span className={"d-flex align-items-center cursor"} style={{ height: "30px", width: "30px" }}>
      {pais == "" ? (
        <svg onClick={() => setsearchPais(!searchPais)} className="mundo" xmlns="http://www.w3.org/2000/svg" height="30" viewBox="0 -960 960 960" width="30">
    ...
    <section>
      <div className="d-flex gap-2 px-1 justify-content-center">
        ...
        <label className="border border-light p-1 d-flex active" htmlFor="location">
          {search}
          <input placeholder="Escriba una ciudad" value={localValue} onChange={InputChange} type="text" className="outline-none text-white-50 w-100 bg-transparent border border-0 ms-2" style={{ outline: "none" }} />
          {paisActive}
        </label>
        {/* Botón de búsqueda */}
        <button onClick={buscar} type="button" className="btn rounded-0 btn-primary" disabled={disabled}>
          Search
        </button>
      </div>
      <div className="px-2">
