...
export default function SearchPais({ fun, active }) {
  // Estado para almacenar los datos de los países
  const [Data, setData] = useState([]);
  // Estado para almacenar los datos filtrados
  const [filtrado, setFiltrado] = useState([]);
  // Estado para el valor local del input
  ...
  // Filtrar la lista de países en base al nombre
  ...
  // Manejar el clic en un elemento de la lista de países filtrados
  ...
  // Limpiar el valor y la lista filtrada
  const limpiar = () => {
    fun("");
    setLocalValue("");
    setFiltrado([]);
  ...
  return (
    <div className="mt-2 px-1 d-flex flex-column h-100 flex-wrap align-items-center">
      ...
      <label style={{ maxWidth: "344px" }} className="border border-light d-flex p-1 w-100 active" htmlFor="location">
        {search}
        <input placeholder="Filtro por país" value={localValue} onChange={InputChange} type="text" className="outline-none text-white-50 w-100 bg-transparent border border-0 ms-2" style={{ outline: "none" }} />
        ...
        {filtrado.map((dato, index) => {
          ...
          return (
            <div key={index} className="d-flex cursor justify-content-between m-3 p-1 border border-light border-opacity-50">
              <span className="pe-auto text-white" onClick={(e) => inputClick(e, dato.abrev)}>
                {dato.nombre}
              </span>
            </div>
          );
